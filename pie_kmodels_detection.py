...
loaders = utils.data.get_ordered_cifar10_validation(batch_size=batch_size, workers=workers, shuffle=shuffle)
...
for models_pruning in [0.3]:  # , 0.5, 0.7, 0.9
    ...
    for input_, label in loaders["valid"]:
        ...
        with torch.set_grad_enabled(False):
            ...
            for model in models:
                # print(input_)  # see if the inputs are the same they must produce the same output
                #
                ...
                print(preds_output_model)
                # print(preds_output_model)

                # for model in pruned_models:  # models
                #    # print(input_)  # see if the inputs are the same they must produce the same output
                #
                #    output_model = model(input_)
                #    print("Output of the model:")
        ...
        print("Modal labels: ")
        print(most_frequent_labels_not_pruned)
        # break
        if debug:
            # logging.info("Result models not pruned: {}".format(results_model_notpruned))
            logging.info("Most frequent not pruned labels: {}".format(most_frequent_labels_not_pruned))
    ...
    if not debug:
        ...
        logging.info(print("Pruned: {} Models used: {} Found PIEs: {}".format(models_pruning, models_number, pie)))
